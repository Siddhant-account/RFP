...
export default function DocumentViewer() {
  ...
  const onDocumentLoadError = useCallback((error: Error) => {
    ...
    setError(
      "Failed to load document. The file might be corrupted or not a valid PDF.",
    );
    setLoading(false);
      ...
      <div className="flex items-center justify-center h-96">
        <div className="text-center max-w-md">
          ...
          <p className="text-brand-600 mb-4">
            This file type ({fileType.toUpperCase()}) cannot be previewed in the
            browser.
          </p>
          <p className="text-sm text-brand-500">File: {fileName}</p>
          <Badge
            variant="secondary"
            className="bg-brand-100 text-brand-700 mt-2"
          >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Button variant="ghost" size="sm" asChild>
                <Link
                  to={`/results?fileUrl=${encodeURIComponent(fileUrl || "")}&fileName=${encodeURIComponent(fileName)}&fileId=${documentId}`}
                >
              ...
              <div>
                ...
                <p className="text-sm text-brand-600">
                  {targetPage &&
                    numPages &&
                    `Viewing page ${targetPage} of ${numPages} • `}
                  {fileName}
                </p>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <Card className="border-brand-200">
              <CardContent className="p-6">{renderFileContent()}</CardContent>
            </Card>
          </div>
            ...
            <Card className="border-brand-200">
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span
                    className="font-medium text-brand-900 text-right text-sm max-w-[150px] truncate"
                    title={fileName}
                  >
                    {fileName}
                  </span>
